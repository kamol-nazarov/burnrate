...
from spend_app.aggregate import _range_fingerprint, _subscription_cost
from spend_app.api import create_app
from spend_app.db import _initialize_base, connect, initialize
from spend_app.plan_service import (
    PlanConflict,
    PlanError,
    apply_mutation,
    effective_terms,
...
from spend_app.subscriptions import daily_cost, materialize_subscription_days
...
def test_effective_history_calendar_cost_and_end_reconciliation(tmp_path):
    ...
    mutate(
        ...
        values={"amount_usd": "150", "cadence": "monthly"},
    )
    ...
    with connect(db) as c:
        ...
        daily = c.execute(
            "SELECT date,cost_usd FROM subscription_daily_costs ORDER BY date"
        ).fetchall()
        assert len(daily) == 20 and daily[-1]["date"] == "2026-09-20"
        assert abs(
            sum(Decimal(str(r["cost_usd"])) for r in daily) - expected
        ) < Decimal("1e-12")
        exact_daily = c.execute(
            "SELECT cost_decimal FROM subscription_daily_costs"
        ).fetchall()
        assert abs(sum(Decimal(r[0]) for r in exact_daily) - expected) < Decimal(
            "1e-24"
        )
        ...
        assert (
            terms[0]["amount_usd"] == "100.0" and terms[0]["end_date"] == "2026-09-14"
        )
...
@pytest.mark.parametrize(
    "day,cadence,days",
    [
        (date(2024, 2, 29), "monthly", 29),
        (date(2024, 2, 29), "quarterly", 91),
        (date(2024, 12, 31), "annual", 366),
        (date(2025, 1, 1), "annual", 365),
        (date(2026, 10, 1), "quarterly", 92),
    ],
)
...
def test_schedule_then_end_before_scheduled_term_preserves_history(tmp_path):
    ...
    mutate(
        ...
        values={"amount_usd": "150", "cadence": "annual"},
    )
...
@pytest.mark.parametrize(
    "amount", ["NaN", "Infinity", "-1", "1000001", "1.234", True, None]
)
...
def test_actual_v9_upgrade_backup_repeatability_and_failure(tmp_path, monkeypatch):
    ...
    with connect(db) as c:
        ...
        c.execute(
            "INSERT INTO subscriptions(name,tool_key,amount_usd,cadence,start_date,end_date) VALUES('Legacy','codex',100,'monthly','2026-09-01','2026-09-20')"
        )
    import spend_app.db as module

    original = module.backup_database
    monkeypatch.setattr(
        module, "backup_database", lambda *a: (_ for _ in ()).throw(OSError("denied"))
    )
    with pytest.raises(RuntimeError, match="backup failed"):
        initialize(db)
    with sqlite3.connect(db) as c:
        assert "plan_id" not in {
            r[1] for r in c.execute("PRAGMA table_info(subscriptions)")
        }
        assert (
            c.execute(
    ...
    backups[0].unlink()  # Disposable backup must not retain a Windows file lock.
...
def test_current_terms_do_not_reenter_legacy_normalization(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        module,
        "_initialize_base",
        lambda p: (_ for _ in ()).throw(AssertionError("legacy normalization called")),
    )
    initialize(db)
...
def test_api_write_boundaries_and_safe_reads(tmp_path, monkeypatch):
    ...
    body = {
        "operation": "add",
        "request_id": "request-add-123",
        "values": values(name="<img src=x onerror=alert(1)>"),
    }
    ...
    assert (
        client.post(
            "/api/subscriptions", headers=headers | {"Host": "evil.example"}, json=body
        ).status_code
        == 403
    )
    assert (
        client.post("/api/subscriptions", headers=headers, data="x").status_code == 415
    )
    assert (
        client.post(
            "/api/subscriptions", headers=headers, json=body | {"secret": "no"}
        ).status_code
        == 422
    )
    ...
    assert (
        client.post("/api/subscriptions", headers=headers, json=body).json()
        == first.json()
    )
    assert len(client.get("/api/subscriptions").json()["plans"]) == 1
    assert (
        client.get("/api/subscriptions").json()["plans"][0]["terms"][0]["name"]
        == body["values"]["name"]
    )


def test_configured_tls_termination_origin(tmp_path, monkeypatch):
    monkeypatch.setenv("BURNRATE_ALLOWED_HOSTS", "dashboard.example.invalid")
    monkeypatch.setenv("BURNRATE_ALLOWED_ORIGINS", "https://dashboard.example.invalid")
    app = create_app(make_settings(tmp_path / "proxy.db"), enable_scheduler=False)
    client = TestClient(app, base_url="http://dashboard.example.invalid")
    headers = {"Origin": "https://dashboard.example.invalid", "X-BURNRATE-Request": "1"}
    body = {"operation": "add", "request_id": "proxy-add-123", "values": values()}
    assert (
        client.post("/api/subscriptions", headers=headers, json=body).status_code == 200
    )
    assert (
        client.post(
            "/api/subscriptions",
...
def test_unknown_existing_schema_requires_backup_and_stops(tmp_path):
    db = tmp_path / "unknown.db"
    with sqlite3.connect(db) as c:
        c.execute("CREATE TABLE unrelated(value TEXT)")
    with pytest.raises(RuntimeError, match="identified"):
        initialize(db)
...
def test_upgrade_reconciliation_failure_rolls_back(tmp_path, monkeypatch):
    db = tmp_path / "rollback.db"
    _initialize_base(db)
    with connect(db) as c:
        c.execute("UPDATE app_meta SET value='9' WHERE key='schema_version'")
        sid = c.execute(
            "INSERT INTO subscriptions(name,tool_key,amount_usd,cadence,start_date) VALUES('Old','codex',100,'monthly','2026-09-01')"
        ).lastrowid
        c.execute(
            "INSERT INTO subscription_daily_costs(subscription_id,tool_key,date,cost_usd,raw_id) VALUES(?,'codex','2026-09-01',1,'old-daily')",
            (sid,),
        )
    monkeypatch.setattr(
        "spend_app.subscriptions.materialize_subscription_days",
