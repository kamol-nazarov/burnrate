...
TOOLS = ("codex", "claude-code", "cursor", "grok", "opencode", "zcode", "openrouter", "xai", "antigravity", "custom")
...
def iso_date(value, optional=False):
    ...
    try:
        ...
        raise PlanError("Use a real ISO date (YYYY-MM-DD), between 1900 and 2200") from None
...
def validate_term(values):
    ...
    try:
        ...
        raise PlanError("USD amount must be finite, between 0 and 1000000, with at most two decimal places") from None
    ...
    return {**values, "name": name.strip(), "tool_key": "opencode" if values["tool_key"] == "zcode" else values["tool_key"],
            "amount_usd": float(amount)}


def create_plan(connection, values):
    term = validate_term(values)
    plan_id = connection.execute("INSERT INTO subscription_plans(end_date) VALUES(?)", (term["end_date"],)).lastrowid
    connection.execute(
        "INSERT INTO subscriptions(plan_id,name,tool_key,amount_usd,cadence,start_date,end_date) VALUES(?,?,?,?,?,?,?)",
        (plan_id, *(term[key] for key in ("name", "tool_key", "amount_usd", "cadence", "start_date", "end_date"))),
    )
    return plan_id
...
def list_plans(connection, today):
    plans = []
    for plan in connection.execute("SELECT id,version,end_date FROM subscription_plans ORDER BY id"):
        terms = [dict(row) for row in connection.execute("SELECT * FROM subscriptions WHERE plan_id=? ORDER BY start_date", (plan["id"],))]
        if not terms:
            continue
        for term in terms:
            term["amount_usd"] = str(Decimal(str(term["amount_usd"])))
            effective_end = min(v for v in (term["end_date"], plan["end_date"]) if v) if term["end_date"] or plan["end_date"] else None
            term["status"] = "ended" if effective_end and (effective_end < today.isoformat() or effective_end < term["start_date"]) else "scheduled" if term["start_date"] > today.isoformat() else "active"
            term["monthly_equivalent"] = str(Decimal(term["amount_usd"]) / {"monthly": 1, "quarterly": 3, "annual": 12}[term["cadence"]])
        plans.append({"id": plan["id"], "version": plan["version"], "end_date": plan["end_date"], "terms": terms})
    return {"plans": plans, "tools": TOOLS, "cadences": CADENCES, "today": today.isoformat(),
...
def effective_terms(connection):
    ...
    rows = []
    for row in connection.execute("SELECT s.*, p.end_date AS plan_end FROM subscriptions s LEFT JOIN subscription_plans p ON p.id=s.plan_id"):
        item = dict(row)
...
def apply_mutation(connection, body, today):
    allowed = {"operation", "request_id", "plan_id", "expected_version", "term_id",
               "values", "effective_date", "end_date", "preview", "confirmed", "preview_token"}
    ...
    operation_fields = {
        ...
        "correct": {"plan_id", "expected_version", "term_id", "values", "preview", "confirmed", "preview_token"},
    }
    ...
    previous = connection.execute("SELECT digest,response FROM subscription_mutations WHERE request_id=?", (request_id,)).fetchone()
    if previous:
        if previous["digest"] != digest:
            raise PlanConflict("This request_id was already used for a different change")
    ...
    if op == "add":
        ...
        plan = connection.execute("SELECT version,end_date FROM subscription_plans WHERE id=?", (plan_id,)).fetchone()
        if plan is None:
            raise PlanError("Plan does not exist")
        if plan[0] != body["expected_version"]:
            raise PlanConflict("This plan changed since you opened it. Reload its history before saving.")
        old = [dict(row) for row in connection.execute("SELECT * FROM subscriptions WHERE plan_id=? ORDER BY start_date", (plan_id,))]
        ...
        if op == "schedule":
            ...
            current = next((t for t in terms if t["start_date"] <= start.isoformat() and (not t["end_date"] or t["end_date"] >= start.isoformat())), None)
            if current is None or current["start_date"] == start.isoformat():
                raise PlanError("A term already starts on this date, or the plan is inactive; use correction")
            values = body.get("values") or {}
            if set(values) != {"amount_usd", "cadence"}:
                raise PlanError("Schedule accepts only amount and cadence")
            new = validate_term({**{k: current[k] for k in FIELDS}, **values, "start_date": start.isoformat()})
            ...
            connection.execute("UPDATE subscription_plans SET end_date=? WHERE id=?", (ending.isoformat() if ending else None, plan_id))
        ...
        if op == "correct":
            ...
            end = max([today] + [iso_date(t["end_date"]) for t in old + terms if t["end_date"]])
            if plan["end_date"]:
                end = min(end, iso_date(plan["end_date"]))
            token = hashlib.sha256(json.dumps([plan_id, plan[0], terms], sort_keys=True).encode()).hexdigest()
            impact = {"from": start.isoformat(), "through": end.isoformat(),
                      "before_usd": str(_sum(old, start, end)), "after_usd": str(_sum(terms, start, end)),
            ...
            if not body.get("confirmed") or body.get("preview_token") != token:
                raise PlanError("Preview and explicitly confirm the historical correction first")
        elif preview:
            raise PlanError("Preview is supported for historical correction only")
        for term in terms:
            if term["id"] is None:
                connection.execute("INSERT INTO subscriptions(plan_id,name,tool_key,amount_usd,cadence,start_date,end_date) VALUES(?,?,?,?,?,?,?)",
                                   (plan_id, *(term[k] for k in ("name", "tool_key", "amount_usd", "cadence", "start_date", "end_date"))))
            else:
                connection.execute("UPDATE subscriptions SET name=?,tool_key=?,amount_usd=?,cadence=?,start_date=?,end_date=? WHERE id=?",
                                   (*(term[k] for k in ("name", "tool_key", "amount_usd", "cadence", "start_date", "end_date")), term["id"]))
        connection.execute("UPDATE subscription_plans SET version=version+1 WHERE id=?", (plan_id,))
        ...
        bounds = connection.execute("SELECT MIN(date),MAX(date) FROM subscription_daily_costs WHERE subscription_id IN (SELECT id FROM subscriptions WHERE plan_id=?)", (plan_id,)).fetchone()
        if bounds[0]:
            from spend_app.subscriptions import materialize_subscription_days
            materialize_subscription_days(connection, start=iso_date(bounds[0]), end=iso_date(bounds[1]))
    connection.execute("INSERT INTO subscription_mutations VALUES(?,?,?)", (request_id, digest, json.dumps(result)))
    return result
