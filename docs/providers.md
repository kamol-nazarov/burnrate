...
| Grok Build quota | same log `billing: fetched credits config` (tail 1 MiB); fallback Traycer CLI `profile-rate-limits grok` | none by BURNRATE if the local snapshot is current | none local; Traycer env on fallback | Local snapshot observed; Traycer path **experimental** | `quotas` `source=grok_local_billing` or `traycer_profile` weekly unit `pct` | Exact % + reset only when observed within 15 minutes. Empty fresh new week → exact 0. Older snapshots are unavailable (not reused). | Read-only log. Traycer CLI spawn on fallback only. |
| OpenCode Grok activity | OpenCode local usage records with `model_key=opencode:grok…` | none | none | Observed local usage route | Capacity note on Grok Build | Shows that OpenCode Grok activity was observed separately. It never supplies or changes the Grok Build percentage because OpenCode exposes no Build allowance or account linkage. | Read-only SQLite ingest. |
